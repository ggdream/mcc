...
func (d *DingTalk) SendPushMessage(_ context.Context, payload *payload.PushPayload) error {
	return d.client.SendActionCardMessage("代码部署: "+payload.Repo.FullName, payload.HeadCommit.Message, dingtalk.WithCardSingleURL(payload.HeadCommit.URL))
}

// SendTagMessage implements Notify.
func (d *DingTalk) SendTagMessage(ctx context.Context, payload *payload.PushPayload) error {
	return d.client.SendActionCardMessage("发版提醒: "+payload.Repo.FullName, payload.HeadCommit.Message, dingtalk.WithCardSingleURL(payload.HeadCommit.URL))
}
